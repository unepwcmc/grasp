pipeline {
   ...
   environment {
        ...
        SLACK_TOKEN = credentials('slack-token-grasp')
        SLACK_CHANNEL = "#jenkins-cicd-grasp"
...
def dockerImageCleanup() {
    ...
    sh '''#!/bin/bash
	docker ps -a --no-trunc  | grep "grasp" | awk '{print $1}' | xargs -r --no-run-if-empty docker stop
	docker ps -a --no-trunc  | grep "grasp" | awk '{print $1}' | xargs -r --no-run-if-empty docker rm -f
	docker images --no-trunc | grep "grasp" | awk '{print $3}' | xargs -r --no-run-if-empty docker rmi -f
    '''    
}

def deleteDeployDir() {
    sh "sudo rm -r $DIR/deploygrasp*"
}
