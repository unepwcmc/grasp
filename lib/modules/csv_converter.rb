class CsvConverter
  ...
  # This module takes a column header and value pair from a csv cell, and adds it to
  # the bjson data attribute of a report.
  # To do this it generates methods defined in the CONVERSIONS constant, named to match the
  # csv column names. Each method adds the value to the correct position in the data hash,
  # or sets the user association on the report.
  # Sometimes the value is validated (e.g. species names, dates, coordinates), and if 
  # the validation fails an error is raised which is caught in the CsvImporter module,
  # the import fails, and the user is notified.
  ...
  CONVERSIONS = {
    ...
    "Date of Discovery"        => proc { |value| answer_date(value) },
  ...
  def answer_date(value)
    invalid_date_error = "Invalid date format. Date must take format of DD/MM/YYYY."
    Date.strptime(value, "%d/%m/%Y") rescue(raise CsvConversionError, invalid_date_error)

    answer("date_of_discovery", value)
  end
